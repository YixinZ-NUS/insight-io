...
Result<void> send_message(int fd, const std::string& payload,
    ...
    if (!payload.empty()) {
        std::memcpy(buffer.data() + sizeof(length), payload.data(),
                    payload.size());
    }
...
Result<SocketMessage> recv_message(int fd, size_t max_payload, size_t max_fds) {
    ...
    const size_t buf_size = sizeof(uint32_t) + max_payload;
    std::string buffer;
    ...
    if (sizeof(uint32_t) + length > static_cast<size_t>(received)) {
        return Result<SocketMessage>::err(
            {"socket_error", "payload truncated"});
    }

    if (length > 0) {
        out.payload.assign(buffer.data() + sizeof(length), length);
    }
    ...
    for (auto* cmsg = CMSG_FIRSTHDR(&msg); cmsg != nullptr;
        ...
        for (size_t i = 0; i < count; ++i) {
            out.fds.push_back(recv_fds[i]);
        }
    }

    return Result<SocketMessage>::ok(std::move(out));
}
